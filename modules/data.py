    ...
    def all_anime(self):
        ...
        while not next_page.startswith('javascript:alert'):
            ...
            for anime in anime_items:
                ...
                # Launched season
                season = pd.to_datetime(anime.select_one('.theme-time').text.replace(
                    '年份：', '').strip()).strftime('%Y-%m')
...
class UpdateSheet:
    ...
    def anime_level(self):
        ...
        df_all_anime['thumbnail'] = df_all_anime['thumbnail'].apply(lambda x: f'=IMAGE("{x}", 4, 100, 80)')
        ...
        frozen_cols = get_frozen_column_count(worksheet) or 4
